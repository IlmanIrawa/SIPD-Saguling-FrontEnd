        ...
        <table class="modern-table">
          ...
          <tbody>
            <tr v-for="pengajuan in spk" :key="pengajuan.pengajuanid">
              ...
              <td>
                <div class="text-muted">{{ formatTanggal(pengajuan.tanggalPengajuan) }}</div>
              </td>
            </tr>
          </tbody>
        </table>
...
export default {
  ...
  methods: {
    ...
    formatTanggal(tanggalPengajuan) {
      if (!tanggalPengajuan) return "";
      return new Date(tanggalPengajuan).toISOString().split("T")[0]; // Hanya ambil bagian YYYY-MM-DD
    },
    openModal(pengajuan) {
